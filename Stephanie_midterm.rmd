...
pd %>% filter(district == "Bandon SD 54") %>% 
  ...
  scale_color_manual(values = c("#A563CA", "#6DA3F9")) +
  geom_line(aes(x = as.numeric(as.factor(academic_year)))) +
  geom_errorbar(aes(ymin = v - v_se, ymax = v + v_se))


#loop through with 2 school districts using pd_subset
...
plots <- pd_subset %>% 
  ...
  mutate(plots = pmap(list(district, data), 
                      ~ggplot(..2, aes(academic_year, v)) +
                        geom_point(color = ) +
                        geom_line(aes(x = as.numeric(as.factor
                                                     (academic_year))),
                                  color = "gray40") +
...
#example from class slide for plot and adding name to plot
...
```{r, eval = FALSE}
#Stephanie comment: this is ranom codes I was playing with, ignore, will be deleted on final version of midterm 
