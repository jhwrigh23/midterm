...
sub_by_district_1 <- sub_by_district %>%
  mutate(gap = map(sub_by_district$data, ~estimate_v("white", "hispanic_latino",
                                  data = .x)),
         v = map_dbl(gap, "v"),
         auc = map_dbl(gap, "auc"),
...
# this gets us results, but for district overall, not by year. 
